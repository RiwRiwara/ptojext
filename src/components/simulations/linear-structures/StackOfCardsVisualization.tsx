...
import { FiPlus, FiMinus, FiRefreshCw, FiArrowUp, FiArrowDown } from 'react-icons/fi';
import { toast } from 'react-hot-toast';
import { LinearDataItem, LinearVisualizationProps } from './types';

export const StackOfCardsVisualization = ({ initialData }: LinearVisualizationProps) => {
  ...
  const handlePush = async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, (1.2 - animationSpeed) * 1000));
    ...
    // Log operation
    setOperationLog(prev => [`Push: ${newItem.value} added to top of stack`, ...prev.slice(0, 9)]);
  ...
  const handlePop = async () => {
    ...
    // Log operation
    setOperationLog(prev => [`Pop: ${poppedItem.value} removed from top of stack`, ...prev.slice(0, 9)]);
  ...
  const handlePeek = () => {
    ...
    // Log operation
    setOperationLog(prev => [`Peek: Current top element is ${data[0].value}`, ...prev.slice(0, 9)]);
  ...
  return (
    <div className="space-y-8">
              ...
              {data.map((item, index) => (
                <motion.div
                  ...
                  style={{
                    ...
                    top: `${index * 30}px`,
                    transform: activeIndex === index ? 'translateY(-20px)' : 'none',
                ...
                {operationLog.length === 0 ? (
                  ...
                  <ul className="space-y-1">
                    {operationLog.map((log, index) => (
                      <li key={index} className="border-b border-gray-100 pb-1 last:border-0">
                        {log}
                      </li>
                    ))}
                  </ul>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <h3 className="text-lg font-medium mb-4">Understanding Stack Data Structure</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <div>
              <h4 className="font-medium text-sm">What is a Stack?</h4>
              <p className="text-sm text-gray-600">
                A stack is a linear data structure that follows the Last In, First Out (LIFO) principle. 
                Like a stack of cards, elements can only be added or removed from the top of the stack.
              </p>
            </div>
            
            <div>
              <h4 className="font-medium text-sm">Key Operations</h4>
              <ul className="list-disc text-sm text-gray-600 pl-5 space-y-1">
                <li><strong>Push:</strong> Add an element to the top of the stack - O(1)</li>
                <li><strong>Pop:</strong> Remove the top element from the stack - O(1)</li>
          ...
          <div className="space-y-3">
            <div>
              ...
              <ul className="list-disc text-sm text-gray-600 pl-5 space-y-1">
                <li>Access is restricted to the topmost element</li>
                <li>Random access is not allowed</li>
                <li>Can have overflow (in fixed-size implementations)</li>
                <li>Requires additional operations for complex tasks</li>
              </ul>
            </div>
